...
const CodeArenaLeaderboard = () => {
    ...
    useEffect(() => {
        const fetchLeaderboard = async () => {
            try {
                setLoading(true);
                const res = await codeArenaApi.getLeaderboard();
    ...
    const filteredData = leaderboardData.filter(entry =>
        ...
        entry.cnid?.toLowerCase().includes(searchQuery.toLowerCase())
    );
                ...
                <div className="flex-1 overflow-y-auto p-4 md:p-8 custom-scrollbar">
                    <div className="max-w-5xl mx-auto flex flex-col gap-6">
                        ...
                        <div className="border border-[#333] bg-[#0A0A0A] p-6 lg:p-8 shadow-2xl relative rounded-sm flex flex-col md:flex-row md:items-end justify-between gap-6 overflow-hidden">
                            ...
                            {myRank && (
                                <div className="relative z-10 bg-[#111] border border-[#333] px-4 py-3 rounded-sm">
                                    <div className="text-[10px] font-mono text-[#666] uppercase tracking-widest mb-1">Your Rank</div>
                                    <div className="flex items-center gap-3">
                                        <span className="text-2xl font-bold text-accent-400">#{myRank.rank}</span>
                                        <div>
                                            <div className="text-xs font-mono text-white">{myRank.tier}</div>
                                            <div className="text-[10px] font-mono text-[#888]">{myRank.score.toLocaleString()} pts</div>
                                        </div>
                                    </div>
                                </div>
                            )}
                        ...
                        <div className="bg-[#0A0A0A] border border-[#222] rounded-sm overflow-hidden mb-20">
                            <table className="w-full text-left border-collapse">
                                ...
                                <tbody>
                                    {loading ? (
                                        <tr>
                                            <td colSpan={5} className="py-8 text-center text-[#888] font-mono text-sm">Loading leaderboard...</td>
                                        </tr>
                                    ) : filteredData.length === 0 ? (
                                        <tr>
                                            <td colSpan={5} className="py-8 text-center text-[#888] font-mono text-sm">No users found</td>
                                        </tr>
                                    ) : (
                                        filteredData.map((data) => (
                                            <tr key={data.rank || data.cnid} className={`border-b border-[#222] hover:bg-[#111] transition-colors ${data.isCurrentUser ? 'bg-accent-500/5' : ''}`}>
                                                ...
                                                <td className="py-4 px-6 font-sans font-bold text-sm text-white">
                                                    {data.user || data.cnid} {data.isCurrentUser && <span className="ml-2 text-[9px] font-mono text-accent-500 uppercase tracking-widest border border-accent-500/30 px-1.5 py-0.5 rounded-sm">You</span>}
                                                </td>
